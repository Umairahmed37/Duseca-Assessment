...
export default function RootLayout({
   ...
   useLayoutEffect(() => {
      ...
      if (session.status === 'unauthenticated') {
         redirect('/login')
      } else if (session.status === 'authenticated' && session.data?.user.role !== Role.USER) {
   ...
   //    redirect('/login')
   // }
   // //REDIRECT USER IF ITS NOT USER
   // if (session.status !== "loading" && session.status === 'authenticated' && session.data?.user.role !== Role.USER) {
   //    toast.error("UnAuthorized Access Not Allowed")
   //    setTimeout(() => {
   //       redirect('/')
   //    }, 500);
